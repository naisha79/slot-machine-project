...
def get_slot_machine_spin(rows, cols, symbols):
    all_symbols = []
    for symbol, count in symbols.items():  # FIX: use .items() not .item()
        for _ in range(count):  # FIX: loop range(count)
            all_symbols.append(symbol)  # FIX: append symbol, not symbols
